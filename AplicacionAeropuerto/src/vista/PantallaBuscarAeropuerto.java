...
public class PantallaBuscarAeropuerto extends JFrame {

	private static final long serialVersionUID = 1L;
	private static boolean instanciaAbierta = false;

	public PantallaBuscarAeropuerto(ControladorAeropuerto controlador) {
		if (instanciaAbierta) {
			return;
		}

		setTitle("Buscar Aeropuerto");
		setSize(600, 400);
		...
		mostrarButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (!nombre.isEmpty()) {
					outputArea.setText("");
					controlador.readAeropuerto(nombre, outputArea);
				} else {
